...
use crate::value::{Closure, EnvironmentChain, Scope, StructData, Value}; // Removed StructData import as it's unused for now
...
impl VirtualMachine {
    pub fn new() -> Self {
        ...
        let initial_env_chain = vec![Scope::Lexical(global_scope_map)];
    ...
    pub fn load_program(
        ...
        let global_scope_map = Rc::new(RefCell::new(HashMap::new()));
        self.current_env_chain = vec![Scope::Lexical(global_scope_map)];

        Ok(())
    }

    #[allow(dead_code)] // Will be used when StoreLocal/LoadLocal are fully implemented
    ...
    fn lookup_variable(&self, name: &Rc<String>) -> Option<Value> {
        // Search current function's indexed locals by name (requires name-to-index map if used this way)
        // This is complex if locals are purely indexed. For now, `LoadLocal` uses index directly.
        // This lookup is primarily for lexical (named) scopes.
        ...
        loop {
            ...
            match instruction {
                BytecodeInstruction::Operation(op) => {
                    match op {
                        ...
                        OpCode::Print => {
                            println!(
                                "Output: {}",
                                self.operand_stack.pop().ok_or_else(|| NsError::Vm(
                                    "PRINT requires a value".to_string()
                                ))?
                            );
                        }
                        OpCode::Return => {
                        ...
                        OpCode::Cons => {
                            let item = self
                                .operand_stack
                                .pop()
                                .ok_or(NsError::Vm("CONS needs item".to_string()))?; // item is popped first
                            let list_val = self
                                .operand_stack
                                .pop()
                                .ok_or(NsError::Vm("CONS needs list".to_string()))?; // then list
                            match list_val {
                                Value::List(list_rc) => {
                ...
                BytecodeInstruction::Call { arity } => {
                    ...
                    match callee_val {
                        Value::Closure(closure_rc) => {
                            ...
                            let new_local_values = Vec::with_capacity(arity); // Does not need to be mutable here

                            let new_local_scope = Scope::Locals(RefCell::new(new_local_values));
                ...
                BytecodeInstruction::StoreLocal(index) => {
                    ...
                    if let Some(Scope::Locals(locals_rc)) = self.current_env_chain.last_mut() {
                        // last_mut for RefCell interior mutability
                ...
                BytecodeInstruction::LoadLocal(index) => {
                    if let Some(Scope::Locals(locals_rc)) = self.current_env_chain.last() {
                        ...
                        return Err(NsError::Vm(
                            "No local scope (Scope::Locals) for LoadLocal".to_string(),
                        ));
                    }
                }
                ...
                BytecodeInstruction::MakeStruct {
                    ...
                    // field_names Vec<String>, type_name String
                    ...
                    // Values are on stack in order of field_names. Pop them in reverse.
                    for field_name_str in field_names.iter().rev() {
                        let value = self.operand_stack.pop().unwrap(); // Should be safe due to len check
                ...
                BytecodeInstruction::SetField(field_name_str) => {
                    if self.operand_stack.len() < 2 {
                        return Err(NsError::Vm(format!(
                            "SET_FIELD '{}' requires a struct instance and a value",
                            field_name_str
                        )));
                    }
                ...
                } // _ => return Err(NsError::Vm(format!("Instr {:?} not implemented", instruction))),
            }
        }
